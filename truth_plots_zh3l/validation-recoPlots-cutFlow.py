...
def getSampleBaseWeight(sample_files):
    # baseW (stored in post-processed nanoAOD files) is a single value for an entire sample and not an event-by-event value
    if isinstance(sample_files[0], list):
        all_files = [f for sublist in sample_files for f in sublist]
    else:
        all_files = sample_files
    # Sum genEventSumw across all files of a sample to get the global total of genWeights
    global_df = ROOT.RDataFrame("Runs", all_files)
    global_sumw = global_df.Sum("genEventSumw").GetValue()
    if global_sumw == 0:
        print("Warning: Global sumw is 0 for this sample!")
        return 1.0

    # Open just ONE file to reverse-calculate the cross section (xs)
    first_file = all_files[0]
    file_df = ROOT.RDataFrame("Runs", first_file)
    file_sumw = file_df.Sum("genEventSumw").GetValue()
    f = ROOT.TFile.Open(first_file)
    t = f.Get("Events")
    t.GetEntry(0)
    basew_local = float(t.baseW)
    f.Close()  
    # (baseW = xs * 1000 / file_sumw, so xs_factor = baseW * file_sumw). the 1000 comes to convert picobarn to femtobarn
    xs_factor = basew_local * file_sumw
    return xs_factor / global_sumw

def addSampleWeight(samples, proc, sample, weight):
    samples[proc]["weights"][sample] = weight
...
parser.add_argument("--i", type=str, default="/eos/user/d/dshekar/MCsamplesForBDTzh3l",
                    help="Path to the directory containing NanoAOD ROOT files.")
parser.add_argument("--lumi", type=float, default=26.7,
                    help="Integrated luminosity in inverse fb or pb^-1 depending on your xsec units.")
args = parser.parse_args()

input_dir = args.i
lumi = args.lumi
print(f"Input directory set to: {input_dir}")
print(f"Luminosity set to: {lumi}")
...
limitFiles = -1
Whad_recoMethodology = 'refined' # 'normal' or 'refined'
FROM_HARD_PROCESS = 8
IS_FIRST_COPY = 12
...
samples = ["GluGluZH_Zto2L_Hto2WtoLNu2Q", "ZH_Zto2L_Hto2WtoLNu2Q", "WZ"] # WZTo3LNu
cutflow_vs_sample = {}    
...
for sample in samples:
    ...
    with ROOT.TFile(str(save_dir)+"/reco_plots_"+str(Whad_recoMethodology)+"WHadReco-cutFlow.root", "RECREATE") as out:
        ...
        if root_files:
            try:
                base_weight = getSampleBaseWeight(root_files)
            except Exception:
                print("Warning: Could not retrieve base weight from ROOT files. Using default base weight of 1.0.")
                base_weight = 1.0
        ...
        for root_file_iter, root_file_name in enumerate(root_files):
            ...
            for event in tree:
                ...
                if hasattr(event, "genWeight"):
                    event_weight *= event.genWeight
                else:
                    print(f"Warning: Event does not have 'genWeight' attribute. Using default gen weight of 1.0.")
                event_weight *= base_weight * lumi
                cutflow_full_file[0] += event_weight # Total events
                Z_candidate = None
                ...
                cutflow_full_file[1] += event_weight # After lepton pre-selections
                ...
                cutflow_full_file[2] += event_weight # After low-mass resonance veto
                ...
                cutflow_full_file[3] += event_weight # After Z mass window selection
                ...
                cutflow_full_file[4] += event_weight # After Zγ veto
                ...
                if len(jets) == 1:
                    cutflow_full_file[5] += event_weight # After jet preselection for 1-jet category
                    # Δφ(l + MET, j) < π/2
                    ...
                    if dphi < math.pi/2:
                        signal_region = True
                        selected_w_pair = (0,-999)  # Mark single jet as W candidate
                elif len(jets) >= 2:
                    cutflow_full_file[5] += event_weight # After jet preselection for 2-jet category
                ...
                if not signal_region:
                    continue 
                cutflow_full_file[6] += event_weight # After signal region selection
                ...
                had_w_p4 = None
                w_i, w_j = selected_w_pair

                # 1-jet signal region: the jet is the hadronic W, if Δφ(l + MET, j) > π/2 then the prior IF statement would not have passed
                had_w_p4 = get_jet_p4(jets[w_i]['pt'], jets[w_i]['eta'], jets[w_i]['phi'], jets[w_i]['mass'])
                if w_j != -999:
                    # 2-jet signal region:
                    had_w_p4 = get_jet_p4(jets[w_i]['pt'], jets[w_i]['eta'], jets[w_i]['phi'], jets[w_i]['mass']) + get_jet_p4(jets[w_j]['pt'], jets[w_j]['eta'], jets[w_j]['phi'], jets[w_j]['mass'])
                
                n_W_had_candidates += 1
                
                if had_w_p4 is not None:
                    phi_W_lep = (lep_p4 + nu_p4).Phi()#lep_p4.Phi()
                    phi_W_had = had_w_p4.Phi()
                    
                    # Visible system: lepton + hadronic W
                    vis_p4 = (lep_p4 + nu_p4) + had_w_p4 #lep_p4 + had_w_p4
                    m_vis = vis_p4.M()
                    pt_vis = vis_p4.Pt()
                    px_vis = vis_p4.Px()
                    py_vis = vis_p4.Py()
                ...
                h_nLeps.Fill(len(n_leptons), event_weight)
                h_pT_lep1.Fill(z_leptons[0]['pt'], event_weight)
                h_pT_lep2.Fill(z_leptons[1]['pt'], event_weight)
                h_pz_lep1.Fill(get_lepton_p4(z_leptons[0]['pt'], z_leptons[0]['eta'], z_leptons[0]['phi'], z_leptons[0]['mass']).Pz(), event_weight)
                h_pz_lep2.Fill(get_lepton_p4(z_leptons[1]['pt'], z_leptons[1]['eta'], z_leptons[1]['phi'], z_leptons[1]['mass']).Pz(), event_weight)
                h_E_lep1.Fill(get_lepton_p4(z_leptons[0]['pt'], z_leptons[0]['eta'], z_leptons[0]['phi'], z_leptons[0]['mass']).E(), event_weight)
                h_E_lep2.Fill(get_lepton_p4(z_leptons[1]['pt'], z_leptons[1]['eta'], z_leptons[1]['phi'], z_leptons[1]['mass']).E(), event_weight)
                h_Zmass.Fill(zcand[2], event_weight)
                h_pT_lep3.Fill(third_lepton['pt'], event_weight)
                h_pz_lep3.Fill(get_lepton_p4(third_lepton['pt'], third_lepton['eta'], third_lepton['phi'], third_lepton['mass']).Pz(), event_weight)
                h_E_lep3.Fill(get_lepton_p4(third_lepton['pt'], third_lepton['eta'], third_lepton['phi'], third_lepton['mass']).E(), event_weight)
                h_nJets.Fill(len(jets), event_weight)
                h_Wlepmass_T.Fill(w_lep_mt, event_weight)
                h_Whadmass.Fill(had_w_p4.M(), event_weight)
                h_pT_jet1.Fill(jets[w_i]['pt'], event_weight)
                h_pz_jet1.Fill(get_jet_p4(jets[w_i]['pt'], jets[w_i]['eta'], jets[w_i]['phi'], jets[w_i]['mass']).Pz(), event_weight)
                h_E_jet1.Fill(get_jet_p4(jets[w_i]['pt'], jets[w_i]['eta'], jets[w_i]['phi'], jets[w_i]['mass']).E(), event_weight)
                if w_j != -999:
                    h_pT_jet2.Fill(jets[w_j]['pt'], event_weight)
                    h_pz_jet2.Fill(get_jet_p4(jets[w_j]['pt'], jets[w_j]['eta'], jets[w_j]['phi'], jets[w_j]['mass']).Pz(), event_weight)
                    h_E_jet2.Fill(get_jet_p4(jets[w_j]['pt'], jets[w_j]['eta'], jets[w_j]['phi'], jets[w_j]['mass']).E(), event_weight)
                h_dPhi_WW.Fill(deltaPhi(phi_W_lep, phi_W_had), event_weight) # NOTE: Check if these variables are out of scope
                h_Hmass_T.Fill(mt_higgs, event_weight) # NOTE: Check if these variables are out of scope

                h_dPhi_ZH.Fill(deltaPhi(phi_Z, phi_H_candidate), event_weight)
            f.Close()
    ...
    print("Histograms saved to reco_plots_"+str(Whad_recoMethodology)+"WHadReco--cutFlow.root")
    with open(str(save_dir)+"/recoPlot_"+str(Whad_recoMethodology)+"_cutFlow.json", "w") as file:
        json.dump(cutflow_vs_sample, file, indent=4, cls=NumpyEncoder)