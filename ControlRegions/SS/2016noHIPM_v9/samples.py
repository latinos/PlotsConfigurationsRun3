...
samples['DY'] = {
    'name': files,
    'weight': mcCommonWeight + '*(nLepton<3)*(abs(Lepton_pdgId[0]*Lepton_pdgId[1]) == 11*13 || abs(Lepton_pdgId[0]*Lepton_pdgId[1]) == 13*13)*( !(Sum(PhotonGen_isPrompt==1 && PhotonGen_pt>15 && abs(PhotonGen_eta)<2.6) > 0))',
    'FilesPerJob': 8,
}
