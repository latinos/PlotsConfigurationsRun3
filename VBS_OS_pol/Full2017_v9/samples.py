...
samples['dyll'] = {
    'name': files,
    'weight': mcCommonWeight + '*( !(Sum(PhotonGen_isPrompt==1 && PhotonGen_pt>15 && abs(PhotonGen_eta)<2.6) > 0))*((Lepton_pdgId[0]*Lepton_pdgId[1] == -11*11) || (Lepton_pdgId[0]*Lepton_pdgId[1] == -13*13))',
    'FilesPerJob': 5,
}
'''
...
###### Zjj EWK #######
'''
files = nanoGetSampleFiles(mcDirectory, 'EWKZ2Jets_ZToLL_M-50_MJJ-120')

samples['Zjj'] = {
        'name': files,
        #'weight': mcCommonWeight + '*(lhe_mjj > 120)',
