...
nuisances['lumi_Uncorrelated'] = {
    ...
    'type': 'lnN',
    'samples': dict((skey, '1.02') for skey in mc if skey not in ['top', 'dytt'])
}

nuisances['lumi_correlated'] = {
    'name': 'lumi_13TeV_correlated',
    'type': 'lnN',
    'samples': dict((skey, '1.009') for skey in mc if skey not in ['top', 'dytt'])
}
...
nuisances['CRSR_accept_DY'] = {
    ...
    'type': 'lnN',
    'samples': {'dytt': '1.02'},
    'cuts': [cut for cut in cuts2j if 'DY' in cut or "dy" in cut],
}
...
samples_dict = dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mc if 'WWewk' not in skey)
samples_dict.update(dict((skey, ['PSWeight[2]*NormTHU_' + skey + '_PS_ISR_Up', 'PSWeight[0]*NormTHU_' + skey + '_PS_ISR_Do']) for skey in mc if skey in ['WWewk_CMWW_LL', 'WWewk_CMWW_LT', 'WWewk_CMWW_TL', 'WWewk_CMWW_TT', 'WWewk_LL', 'WWewk_LT', 'WWewk_TL', 'WWewk_TT']))
nuisances['PS_ISR']  = {
    'name'    : 'ps_isr',
    'kind'    : 'weight',
    'type'    : 'shape',
...
samples_dict = dict((skey, ['PSWeight[3]', 'PSWeight[1]']) for skey in mc if 'WWewk' not in skey)
samples_dict.update(dict((skey, ['PSWeight[3]*NormTHU_' + skey + '_PS_FSR_Up', 'PSWeight[1]*NormTHU_' + skey + '_PS_FSR_Do']) for skey in mc if skey in ['WWewk_CMWW_LL', 'WWewk_CMWW_LT', 'WWewk_CMWW_TL', 'WWewk_CMWW_TT', 'WWewk_LL', 'WWewk_LT', 'WWewk_TL', 'WWewk_TT']))
nuisances['PS_FSR']  = {
    'name'    : 'ps_fsr',
    'kind'    : 'weight',
    'type'    : 'shape',
