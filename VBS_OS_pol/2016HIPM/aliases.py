...
#KELLO changed for local maps
#KELLO patch might be needed for some version of LGC: include this in lines to process: "ROOT.gSystem.Load('libcorrectionlib.so');"
#btv_path = os.environ["STARTPATH"].replace('start.sh', '') + 'mkShapesRDF/processor/data/jsonpog-integration/POG/BTV/' + year
btv_path = f'{configurations}'
...
for flavour in ['bc', 'light']:
    for shift in ['central', 'up_uncorrelated', 'down_uncorrelated', 'up_correlated', 'down_correlated']:
        ...
        aliases[btagsf] = {
            ...
            'linesToProcess': [f"ROOT.gInterpreter.ProcessLine('btagSF{flavour} btagSF{flavour}_{shift} = btagSF{flavour}(\"{btv_path}/bTagEff_2016_ttbar_DeepFlavB_loose.root\", \"{year}\");')"],
...
#  deprecated SF shape correction
'''
aliases['bVetoSF'] = {
