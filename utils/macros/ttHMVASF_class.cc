...
// protected:
  ...
  double operator()(RVecF Lepton_pt, RVecF Lepton_eta, RVecI Lepton_pdgId) {
	...
	// std::cout << "Starting actual SF application" << std::endl;
    ...
    if(requested_SF_.compare("nominal") == 0) { 
	  ...
	  // std::cout << "ttHMVA ele SF up: " << SF_up_out_el << std::endl;
      return SF_up_out_el; 
    }
    else if(requested_SF_.compare("muUp") == 0) { 
	  // std::cout << "ttHMVA muon SF up: " << SF_up_out_mu << std::endl;
      return SF_up_out_mu; 
    }
    else if(requested_SF_.compare("eleDown") == 0) { 
	  // std::cout << "ttHMVA ele SF down: " << SF_down_out_el << std::endl;
      return SF_down_out_el; 
    }
    else if(requested_SF_.compare("muDown") == 0) { 
	  // std::cout << "ttHMVA muon SF down: " << SF_down_out_mu << std::endl;
...
ttHMVASF::ttHMVASF( const char* year, const int nLeptons, const char* muOrEle, std::string requested_SF ) {
  ...
  SF_files_map["electron"]["2016HIPM"] = {
	mkShapesRDF_base + "/electron_ttHMVA_UL_SF/2016HIPM/egammaEffi.txt"
  };
  SF_files_map["electron"]["2016noHIPM"] = {
	mkShapesRDF_base + "/electron_ttHMVA_UL_SF/2016noHIPM/egammaEffi.txt"
  };
  SF_files_map["electron"]["2017"] = {
	mkShapesRDF_base + "/electron_ttHMVA_UL_SF/2017/egammaEffi.txt"
  };
  SF_files_map["electron"]["2018"] = {
	mkShapesRDF_base + "/electron_ttHMVA_UL_SF/2018/egammaEffi.txt"
  };

  // Muon
  SF_files_map["muon"]["2016HIPM"] = {
	mkShapesRDF_base + "/muon_ttHMVA_SF/2016HIPM/NUM_TightHWW_ISO_tthmva_DEN_TightHWW_ISO_eta_pt.root"
  };
  SF_files_map["muon"]["2016noHIPM"] = {
	mkShapesRDF_base + "/muon_ttHMVA_SF/2016noHIPM/NUM_TightHWW_ISO_tthmva_DEN_TightHWW_ISO_eta_pt.root"
  };
  SF_files_map["muon"]["2017"] = {
	mkShapesRDF_base + "/muon_ttHMVA_SF/2017/NUM_TightHWW_ISO_tthmva_DEN_TightHWW_ISO_eta_pt.root"
  };
  SF_files_map["muon"]["2018"] = {
	mkShapesRDF_base + "/muon_ttHMVA_SF/2018/NUM_TightHWW_ISO_tthmva_DEN_TightHWW_ISO_eta_pt.root"
  };
  ...
  if (strncmp(muOrEle_,"ele",3) || strncmp(muOrEle_,"all",3)){
	...
	std::cout << "SF files map = " << SF_files_map_["electron"][year_] << std::endl;
	...
	std::cout << "Done with loading electrons scale factors" << std::endl;
  }
  ...
  if (strncmp(muOrEle_,"mu",2) || strncmp(muOrEle_,"all",3)){
	...
	std::cout << "Muons rootfile: " << SF_files_map_["muon"][year_] << endl;
    ...
    rootfile->Close();
	std::cout << "Done with loading muons scale factors" << std::endl;
  }
}
