...
class DatacardFactory:
    ...
    # _____________________________________________________________________________
    def makeDatacardsParam( self, inputFile, outputDirDatacard, variables, cuts, samples, structureFile, nuisances, parametric=False):
        ...
        for cutName in cuts:
          ...
          for snameList, idmap in [(cut_signals, signal_ids), (cut_backgrounds, background_ids)]:
            for sampleName in idmap:
              if 'parametric' in structureFile[sampleName]:
                # remove from the list
                print ' remove ', sampleName, ' --> parametric pdf'
          ...
          for variableName, variable in variables.iteritems():
            ...
            # Construct the RooRealVar using one of the TH1s (the first one in the list)
            if (self._parametric):
              templateHisto = self._getHisto(cutName, variableName, samples.keys()[0])
              self.x = ROOT.RooRealVar("x", "x", templateHisto.GetXaxis().GetXmin(), templateHisto.GetXaxis().GetXmax() )
              self.w = ROOT.RooWorkspace("w", "workspace")
...
#                        histo.SetBinContent(iBin,0.)
                ...
                # If parametric datacard, convert all histograms into RooDataHist and save them in a workspace
                if (self._parametric):
                  self._outFile.cd()
                  dh = ROOT.RooDataHist(sampleName, sampleName, ROOT.RooArgList(self.x), histo)
                  getattr(self.w,'import')(dh)
            ...
            for signalName in alternative_signals:
              ...
              if (self._parametric):
                card.write('shapes  *           * '+
                           ...
                           '     w:$PROCESS w:$PROCESS_$SYSTEMATIC' + '\n')
              else:  
                card.write('shapes  *           * '+
                           'shapes/histos_' + tagNameToAppearInDatacard + ".root" +
              ...
              #if (self._parametric) signals.append('sig')
            ...
            if (self._parametric):
              # This should be triggered by the presence of the additional dictionary parametric_shapes.py
              if (True):
                 #sig = self._buildPdf('CrystalBall',x)
                 MH = ROOT.RooRealVar("MH", "mean of gaussian", 50, 0, 1000)
                 sigma = ROOT.RooRealVar("sigma", "width of gaussian", 20, 1, 50)
                 alpha = ROOT.RooRealVar("alpha", "location of transition to a power law", -1., -10., 10.)
                 n = ROOT.RooRealVar("n", "exponent of power-law tail", 1, 0.1, 50)
                 sig = ROOT.RooCBShape("sig", "RooSBShape", self.x, MH, sigma, alpha, n)
                 getattr(self.w,'import')(sig)
              print "Writing workspace to file ", outputDirDatacard + "/" + cutName + "/" + variableName + "/shapes/histos_" + tagNameToAppearInDatacard + ".root"
              self.w.writeToFile(outputDirDatacard + "/" + cutName + "/" + variableName + "/shapes/histos_" + tagNameToAppearInDatacard + ".root")
        ...
        if type(self._fileIn) is dict:
          ...
          self._fileIn.Close()
        print "~~~~~~~~~~~~~~~~~ End of makeDatacardsParam..."
    # _____________________________________________________________________________
    def _saveNuisanceHistos(self, cutName, variableName, sampleName, suffixIn, suffixOut = None, symmetrize = False):
        ...
        if (self._parametric):
            dhUp = ROOT.RooDataHist(histoUp.GetName().replace('histo_',''), histoUp.GetName().replace('histo_',''), ROOT.RooArgList(self.x), histoUp)
            dhDown = ROOT.RooDataHist(histoDown.GetName().replace('histo_',''), histoDown.GetName().replace('histo_',''), ROOT.RooArgList(self.x), histoDown)
            getattr(self.w,'import')(dhUp)
            getattr(self.w,'import')(dhDown)
        else:
            self._outFile.cd()
            histoUp.Write()
            histoDown.Write()
    ...
    # _____________________________________________________________________________
    def _buildPdf(self, name, x):
	if (name=='CrystalBall'):
             MH = ROOT.RooRealVar("MH", "mean of gaussian", 400, 100, 1000)
             sigma = ROOT.RooRealVar("sigma", "width of gaussian", 20, 1, 100)
             alpha = ROOT.RooRealVar("alpha", "location of transition to a power law", -1., -10, 10)
             n = ROOT.RooRealVar("n", "exponent of power-law tail", 1, 0.1, 10)
             sig = ROOT.RooCBShape("sig", "RooCBShape", x, MH, sigma, alpha, n)
             return sig
        else:
             raise RuntimeError("The selected pdf "+name+" is not implemented.")
...
'''    
    ...
    parser.add_option('--isParametric', dest='isParametric', help="Set to True in case of parametric datacards", default=False, action='store_true')
    ...
    factory.makeDatacardsParam( opt.inputFile ,opt.outputDirDatacard, variables, cuts, samples, structure, nuisances, opt.isParametric)
