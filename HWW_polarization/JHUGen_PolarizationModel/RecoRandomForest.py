...
def createInputBranches(inFile, outDir):
    ...
    ROOT.gSystem.Load("/eos/user/s/sblancof/Run2Analysis/mkShapesRDF/examples/extended/RecoMoMEMta_VBF_class_cc.so","", ROOT.kTRUE)
    ...
    if "2018" in baseFolder.split("/")[-1]:
        ROOT.gInterpreter.Declare('EvaluateRF rf_evaluator("2018");')
    elif "2017" in baseFolder.split("/")[-1]:
        ROOT.gInterpreter.Declare('EvaluateRF rf_evaluator("2017");')
    elif "2016" in baseFolder.split("/")[-1]:
        ROOT.gInterpreter.Declare('EvaluateRF rf_evaluator("2016");')

    if "Dyemb" in fileName:
        folders = ["EmbElepT", "EmbMupT"]
    else:
        folders = ["ElepT", "MupT", "RDF__JES", "JER", "MET"]
    varys = ["ElepT", "MupT", "JER", "MET", 'JESAbsolute','JESAbsolute_YEAR','JESBBEC1','JESBBEC1_YEAR','JESEC2','JESEC2_YEAR','JESFlavorQCD','JESHF','JESHF_YEAR','JESRelativeBal','JESRelativeSample_YEAR']


    ###### Exceptions
    exceptFolders = []
    excepts = []
    if "2018" in baseFolder:
        ### Corrupted files
        if ("GluGluToWWToMNEN__part3" in fileName) or ("GluGluToWWToMNEN__part4" in fileName):
            exceptFolders.append("RDF__JES")
            excepts.append('JESAbsolute')
            excepts.append('JESAbsolute_2018')
            excepts.append('JESBBEC1')
            excepts.append('JESBBEC1_2018')
            excepts.append('JESEC2')
            excepts.append('JESEC2_2018')
            excepts.append('JESFlavorQCD')
            excepts.append('JESHF')
            excepts.append('JESHF_2018')
            excepts.append('JESRelativeBal')
            excepts.append('JESRelativeSample_2018')
    elif "2017" in baseFolder:
        if ("GluGluToWWToMNEN__part2" in fileName) or ("GluGluToWWToMNEN__part3" in fileName) or ("GluGluToWWToMNMN__part1" in fileName):
            exceptFolders.append("RDF__JES")
            excepts.append('JESAbsolute')
            excepts.append('JESAbsolute_2017')
            excepts.append('JESBBEC1')
            excepts.append('JESBBEC1_2017')
            excepts.append('JESEC2')
            excepts.append('JESEC2_2017')
            excepts.append('JESFlavorQCD')
            excepts.append('JESHF')
            excepts.append('JESHF_2017')
            excepts.append('JESRelativeBal')
            excepts.append('JESRelativeSample_2017')
    elif "2016_noHIPM" in baseFolder:
        if ("GluGluToWWToENMN__part4" in fileName) or ("GluGluToWWToMNEN__part0" in fileName):
            exceptFolders.append("RDF__JES")
            excepts.append('JESAbsolute')
            excepts.append('JESAbsolute_2016')
            excepts.append('JESBBEC1')
            excepts.append('JESBBEC1_2016')
            excepts.append('JESEC2')
            excepts.append('JESEC2_2016')
            excepts.append('JESFlavorQCD')
            excepts.append('JESHF')
            excepts.append('JESHF_2016')
            excepts.append('JESRelativeBal')
            excepts.append('JESRelativeSample_2016')
    ...
    for syst in varys:        
        ...
        if syst.startswith("JES") and "YEAR" in syst:
            if "2018" in baseFolder.split("/")[-1]:
                syst = syst.replace("YEAR", "2018")
            elif "2017" in baseFolder.split("/")[-1]:
                syst = syst.replace("YEAR", "2017")
            elif "2016" in baseFolder.split("/")[-1]:
                syst = syst.replace("YEAR", "2016")
    ...
    df.df.Snapshot("Events", tmpFile, filteredColumnNames, opts) # RECREATE
