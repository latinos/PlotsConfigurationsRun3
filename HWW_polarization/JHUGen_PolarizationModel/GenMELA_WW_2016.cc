...
public:
  ...
  GEN_POLARIZATION_2016(float f_L){
  ...
  std::vector<RVecF> operator()(int _nLHEPart,
    ...
    for (int p = 0; p < GenPart_pt.size(); p++){
      TLorentzVector tmp;
      //if (GenPart_status[p] == 1 || GenPart_status[p] == 23){ // outgoing
      
      //cout << "Is stable!" << endl;
      //cout << LHEPart_pdgId[p] << endl;
      //cout << LHEPart_mass[p] << endl;
      
      int mother_id = GenPart_pdgId[GenPart_genPartIdxMother[p]];
      int mother_mother_id = GenPart_pdgId[GenPart_genPartIdxMother[GenPart_genPartIdxMother[p]]];
	
      if (abs(GenPart_pdgId[p])>=11 && abs(GenPart_pdgId[p])<=16){
	areLeptons = true;
      }
      
      tmp.SetPtEtaPhiM(GenPart_pt[p], GenPart_eta[p], GenPart_phi[p], GenPart_mass[p]);
      if (first && abs(GenPart_pdgId[p])<17 && abs(GenPart_pdgId[p])>10 && abs(mother_id)==24){
	Higgs = tmp;
	first = false;
	outgoing.push_back(tmp);
	outgoingIDs.push_back(GenPart_pdgId[p]);
	countLeptons++;
      }else if (abs(GenPart_pdgId[p])<17 && abs(GenPart_pdgId[p])>10 && abs(mother_id)==24){
	Higgs = Higgs + tmp;
	outgoing.push_back(tmp);
	outgoingIDs.push_back(GenPart_pdgId[p]);
	countLeptons++;
      }
      //if (GenPart_pdgId[p] == 25){
      //outgoing.push_back(tmp);
      //outgoingIDs.push_back(GenPart_pdgId[p]);
      //}
      //}
    ...
    if (!genCand || countLeptons<=3){
      cout << "Problem here!" << endl;
      std::cout << "Outgoing particles -> " << std::endl;
      for (int i = 0; i<outgoingIDs.size(); i++){
	std::cout << outgoingIDs[i] << std::endl;
      }
      ...
      RVecF dummy = {-999.9};
      RVecF otherVars = {-999.9, -999.9, -999.9, -999.9, -999.9, -999.9, -999.9, -999.9, -999.9};
