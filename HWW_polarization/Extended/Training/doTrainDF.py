...
for i in fnames_2016[0:10]:
    input_bkg.append(i)

for i in fnames_2017[0:10]:
    input_bkg.append(i)

for i in fnames_2018[0:10]:
    input_bkg.append(i)
...
df_bkg = df_bkg.Filter("Lepton_pdgId[0]*Lepton_pdgId[1] == -11*13 && Lepton_pt[0]>25 && Lepton_pt[1]>15 && Alt(Lepton_pt,2, 0)<10 && ptll>15 && mll > 12 && Alt(CleanJet_pt, 0, 0)<30. && Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Take(Jet_btagDeepFlavB, CleanJet_jetIdx) > 0.0532) == 0 && mth>40 && PuppiMET_pt>20 && mll > 12 && mpmet>15")
...
#df_bkg = df_bkg.Filter("Lepton_pdgId[0]*Lepton_pdgId[1] == -11*13 && Lepton_pt[0]>25 && Lepton_pt[1]>15 && Alt(Lepton_pt,2, 0)<10 && ptll>15 && mll > 12 && Alt(CleanJet_pt, 0, 0)>30.0 && Alt(CleanJet_pt, 1, 0)<30.0 && Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Take(Jet_btagDeepFlavB, CleanJet_jetIdx) > 0.0532) == 0 && mth>40 && PuppiMET_pt>20 && mll > 12 && mpmet>15")
...
#df_bkg = df_bkg.Filter("Lepton_pdgId[0]*Lepton_pdgId[1] == -11*13 && Lepton_pt[0]>25 && Lepton_pt[1]>15 && Alt(Lepton_pt,2, 0)<10 && ptll>15 && mll > 12 && Sum(CleanJet_pt>30.)==2 && Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Take(Jet_btagDeepFlavB, CleanJet_jetIdx) > 0.0532) == 0 && mth>40 && PuppiMET_pt>20 && mll > 12 && mpmet>15")
...
df_bkg = df_bkg.Define("lep_phi1", "Lepton_phi[0]")

df = df.Define("lep_phi2", "Lepton_phi[1]")
df_bkg = df_bkg.Define("lep_phi2", "Lepton_phi[1]")
...
df_bkg = df_bkg.Define("btagDeepFlavB", "Alt(Jet_btagDeepFlavB, Alt(CleanJet_jetIdx, 0, -1), -2.0)") 
df_bkg = df_bkg.Define("btagDeepFlavB_1", "Alt(Jet_btagDeepFlavB, Alt(CleanJet_jetIdx, 1, -1), -2.0)")
...
df_bkg = df_bkg.Define("MELA_VBF", "a(nCleanJet, nLepton, PuppiMET_pt, PuppiMET_phi, Lepton_pt, Lepton_phi, Lepton_eta, CleanJet_pt, CleanJet_phi, CleanJet_eta, Lepton_pdgId)")

df = df.Define("D_VBF_QCD", "MELA_VBF[0]")
df = df.Define("D_VBF_VH", "MELA_VBF[1]")
df = df.Define("D_QCD_VH", "MELA_VBF[2]")

df_bkg = df_bkg.Define("D_VBF_QCD", "MELA_VBF[0]")
df_bkg = df_bkg.Define("D_VBF_VH", "MELA_VBF[1]")
df_bkg = df_bkg.Define("D_QCD_VH", "MELA_VBF[2]")
...
df = df.Define("D_VBF_DY", "b(nCleanJet, nLepton, PuppiMET_pt, PuppiMET_phi, Lepton_pt[0], Lepton_pt[1], Lepton_phi[0], Lepton_phi[1], Lepton_eta[0], Lepton_eta[1], CleanJet_pt[0], CleanJet_pt[1], CleanJet_phi[0], CleanJet_phi[1], CleanJet_eta[0], CleanJet_eta[1], Lepton_pdgId[0], Lepton_pdgId[1])") 
df_bkg = df_bkg.Define("D_VBF_DY", "b(nCleanJet, nLepton, PuppiMET_pt, PuppiMET_phi, Lepton_pt[0], Lepton_pt[1], Lepton_phi[0], Lepton_phi[1], Lepton_eta[0], Lepton_eta[1], CleanJet_pt[0], CleanJet_pt[1], CleanJet_phi[0], CleanJet_phi[1], CleanJet_eta[0], CleanJet_eta[1], Lepton_pdgId[0], Lepton_pdgId[1])")
...
df_bkg = df_bkg.Define("Ctot", "detajj!=0 ? log((abs(2 * Lepton_eta[0] - CleanJet_eta[0] - CleanJet_eta[1]) + abs(2 * Lepton_eta[1] - CleanJet_eta[0] - CleanJet_eta[1])) / detajj) : -1")
...
new_indx_LL = random.choices(DF_LL.index, weights=DF_LL['Higgs_WW_LL'].values, k=int(0.7*len(DF_LL)))
new_indx_TT = random.choices(DF_TT.index, weights=DF_TT['Higgs_WW_TT'].values, k=int(0.7*len(DF_TT)))
...
#df_bkg.Snapshot("Events", "ntuples_bkg_2j.root", var)
#df_ll.Snapshot("Events", "ntuples_ll_2j.root", var)
#df_tt.Snapshot("Events", "ntuples_tt_2j.root", var)
...
#Bkg = Bkg[0:len(LL)]
Bkg = Bkg.sample(len(LL))


print("Statistics for Sig0: " + str(len(LL)))
print("Statistics for Sig1: " + str(len(TT)))
print("Statistics for Bkg0: " + str(len(Bkg)))

df_all = pd.concat([LL,TT,Bkg])


#### Normalize columns for DNN -------------------------

print("DO RESHAPE OF INPUT VARIABLES:")
for column in df_all.columns:
  if column in ['isLL', 'isTT', 'isBkg']:
    continue
  
  print(column)
  min_value = np.min(df_all[column].values)
  max_value = np.max(df_all[column].values)

  reshape = (np.array(df_all[column].values) - min_value) / (max_value - min_value)
  df_all[column] = reshape
...
model.add(Dense(256, activation='relu', input_dim=len(var)))
...
model.compile(loss='categorical_crossentropy', optimizer=optimizers.RMSprop(0.00015), metrics=['accuracy'])
n_epochs = 300
n_batch = 512
...
#joblib.dump(rfc, 'random_forest_Categorical_vbf_df.pkl')
...
#discriminant = np.squeeze(np.asarray(y_pred[0][:, 1]))
discriminant = np.squeeze(np.asarray(y_pred_L))
true_labels = np.squeeze(np.asarray(Y_test['isLL']))
true_labels_2 = np.squeeze(np.asarray(Y_test['isTT']))

discriminant0 = discriminant[np.array(true_labels == 0) * np.array(true_labels_2 == 0)]
discriminant1 = discriminant[list(true_labels == 1)]
discriminant2 = discriminant[np.array(true_labels == 0) * np.array(true_labels_2 == 1)]
...
pdf2, bins2, patches2 = plt.hist(discriminant2, bins = binning, color = 'g', alpha = 0.3, histtype = 'stepfilled', linewidth = 1, edgecolor='g', label = 'Higgs to TT')
plt.legend(loc = 'upper center')
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant (LL category)', fontsize = 12)
plt.savefig('Discriminant_distribution_DF_VBF_LL.png', dpi = 600)
...
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant (LL category)', fontsize = 12)
plt.savefig('Log_Discriminant_distribution_DF_VBF_LL.png', dpi = 600)
...
true_labels_2 = np.squeeze(np.asarray(Y_test['isLL']))
...
pdf2, bins2, patches2 = plt.hist(discriminant2, bins = binning, color = 'g', alpha = 0.3, histtype = 'stepfilled', linewidth = 1, edgecolor='g', label = 'Higgs to LL')
plt.legend(loc = 'upper center')
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant (TT category)', fontsize = 12)
plt.savefig('Discriminant_distribution_DF_VBF_TT.png', dpi = 600)
...
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant (TT category)', fontsize = 12)
plt.savefig('Log_Discriminant_distribution_DF_VBF_TT.png', dpi = 600)
...
plt.savefig('Discriminant_distribution_DF_VBF_Bkg.png', dpi = 600)
...
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant (Bkg category)', fontsize = 12)
plt.savefig('Log_Discriminant_distribution_DF_VBF_Bkg.png', dpi = 600)
...
plt.savefig('ROC_RandomForest_DF_VBF.png', dpi = 600)
