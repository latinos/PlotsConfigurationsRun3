...
#df = df.Filter("Lepton_pdgId[0]*Lepton_pdgId[1] == -11*13 && Lepton_pt[0]>25 && Lepton_pt[1]>15 && Alt(Lepton_pt,2, 0)<10 && ptll>15 && mll > 12 && Sum(CleanJet_pt>30.0)>=2 && Alt(CleanJet_pt, 1, 0)<30.0 && Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Take(Jet_btagDeepFlavB, CleanJet_jetIdx) > 0.0532) == 0 && mth>40 && PuppiMET_pt>20 && mll > 12 && mpmet>15")
...
df_bkg = df_bkg.Filter("Lepton_pdgId[0]*Lepton_pdgId[1] == -11*13 && Lepton_pt[0]>25 && Lepton_pt[1]>15 && Alt(Lepton_pt,2, 0)<10 && ptll>15 && mll > 12 && Alt(CleanJet_pt, 0, 0)>30.0 && Sum(CleanJet_pt>30.0)==2 && Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Take(Jet_btagDeepFlavB, CleanJet_jetIdx) > 0.0532) == 0 && mth>40 && PuppiMET_pt>20 && mll > 12 && mpmet>15")
...
df_bkg = df_bkg.Define("D_VBF_DY", "b(nCleanJet, nLepton, PuppiMET_pt, PuppiMET_phi, Lepton_pt[0], Lepton_pt[1], Lepton_phi[0], Lepton_phi[1], Lepton_eta[0], Lepton_eta[1], CleanJet_pt[0], CleanJet_pt[1], CleanJet_phi[0], CleanJet_phi[1], CleanJet_eta[0],\
 CleanJet_eta[1], Lepton_pdgId[0], Lepton_pdgId[1])")
...
df.Snapshot("Events", "ntuples_Sig_binary_2j.root", var)
df_bkg.Snapshot("Events", "ntuples_bkg_binary_2j.root", var)

print("DONE!")
print("Create pandas dataframes!")

dfAll = df.AsNumpy(var)
dfBkg = df_bkg.AsNumpy(var)

Sig = pd.DataFrame(dfAll)
...
X_train, X_test, Y_train, Y_test = train_test_split(df_all[var], df_all[['isSig']], test_size=0.2, random_state=6)
...
#joblib.dump(rfc, 'random_forest_Categorical_0j_df_bkg.pkl')
...
pdf1, bins1, patches1 = plt.hist(discriminant1, bins = binning, color = 'b', alpha = 0.3, histtype = 'stepfilled', linewidth = 1, edgecolor='b', label = 'Higgs')
plt.legend(loc = 'upper center')
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant', fontsize = 12)
plt.savefig('Discriminant_distribution_DF_2j_Bkg.png', dpi = 600)
...
plt.ylabel('Entries', fontsize = 12)
plt.xlabel('Random Forest discriminant', fontsize = 12)
plt.savefig('Log_Discriminant_distribution_DF_2j_Bkg.png', dpi = 600)
...
plt.savefig('ROC_RandomForest_DF_2j_Bkg.png', dpi = 600)
