...
public:
  ...
  RVecF operator()(int _nLHEPart,
    ...
    for (int p = 0; p < GenPart_pt.size(); p++){
      TLorentzVector tmp;
      if (GenPart_status[p] == 1){ // outgoing

	//cout << "Is stable!" << endl;
	//cout << GenPart_pdgId[p] << endl;
	//cout << GenPart_mass[p] << endl;

	if (abs(GenPart_pdgId[p])<17 && abs(GenPart_pdgId[p])>10){
	  areLeptons = true;
	}
	
	tmp.SetPtEtaPhiM(GenPart_pt[p], GenPart_eta[p], GenPart_phi[p], GenPart_mass[p]);
	if (first && abs(GenPart_pdgId[p])<17 && abs(GenPart_pdgId[p])>10){
	  Higgs = tmp;
	  first = false;
	}else if (abs(GenPart_pdgId[p])<17 && abs(GenPart_pdgId[p])>10){
	  Higgs = Higgs + tmp;
	}
    ...
    if (!genCand){
      ...
      RVecF results = {-999.9, -999.9, -999.9, -999.9, -999.9, -999.9, -999.9};
      return results;
    }
    ...
    if (!doVBF){ // skip VBF computation
      ...
      if (GG_SIG_kappaTopBot_1_ghz1_1_MCFM<-99.9){
	genMass = -999.9;
      }else{
    ...
    if (GG_SIG_kappaTopBot_1_ghz1_1_MCFM<-99.9 && JJEW_SIG_ghv1_1_MCFM<-99.9){
      genMass = -999.9;
    }else{
      genMass = (float)genCand->m();
    }
