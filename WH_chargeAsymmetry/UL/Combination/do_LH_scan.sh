...
python3 ../scripts/script_workspace_and_fit.py \
		...
		--output_name=Combination/FitResults_WH_FullRun2_v9_binning_WH_strength.txt \
        --freeze_nuisances=r_higgs --sanity_check=rA_scan \
		--only_fit 1
		
# Freezing all systematic effects and leaving only the statistical uncertainty:
python3 ../scripts/script_workspace_and_fit.py \
		--datacard_name=Combination/WH_chargeAsymmetry_WH_FullRun2_v9_binning_WH_strength \
		--output_name=Combination/FitResults_WH_FullRun2_v9_binning_WH_strength_freeze_all.txt \
		--freeze_nuisances all \
		--sanity_check=rA_scan \
		--only_fit 1
